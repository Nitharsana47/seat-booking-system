...
1. The backend issues a Redis atomic command:
   `SET seat:<eventId>:<seatId> hold:<userId> NX EX 600`
2. **Success (`NX` condition met)**: The seat is locked for **10 minutes**. A Socket.IO event `seat_update` is broadcasted immediately:
   `{ eventId, seatId, status: 'held', userId }`
3. **Failure (`NX` condition not met)**: If the seat is already held or booked, the atomic operation fails, and the backend returns a `409 Conflict`.
4. If the 10 minutes expire without checkout:
   - Redis fires an expired event.
   - The subscriber thread receives it, queries the DB to see if it was a waitlist offer, and if not, broadcasts the seat as `available` to all users via Socket.IO.
...
├── system_design.md             # 800-word concurrency & design writeup
...
2. The `.env` file is pre-configured with local default values:
   ```env
   PORT=5000
   ...
   EMAIL_PASS=
   ```
3. Run install and start the dev server:
   ```bash
   npm install
   npm run dev
   ```
   *Note: On startup, the database schema will apply automatically and seed three default accounts with the password `password123`:*
...
2. Run install and start the dev server:
   ```bash
